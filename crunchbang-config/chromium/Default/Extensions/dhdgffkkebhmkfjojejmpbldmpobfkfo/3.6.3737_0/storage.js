...
});return u},setTemporary:function(u){d=!u;r=true}};
return{isSupported:function(u){u(true)},isWorking:function(u){chrome.storage.local.get(null,function(){var v=!chrome.runtime||!chrome.runtime.lastError;
if(!v&&chrome.runtime.lastError){console.warn("storage:",chrome.runtime.lastError.message)
}u(v)})},init:function(v){if(!p){var u=function(x){p={};
for(var w in x){p[w]=x[w]}if(!t){chrome.storage.onChanged.addListener(q);
t=true}if(v){v()}};chrome.storage.local.get(null,u)
}else{if(v){v()}}},clean:function(u){p=null;if(u){u()
}},options:{},methods:s}})(),file:(function(){var r=null;
var t=null;var A=function(B){return B};var w=function(B){return B
};var v=function(B){var C=function(D){console.warn("fileStorage: listFiles() error:",D);
...
}}}};Registry.register("storage","3737.6",function(){return m
})})})();